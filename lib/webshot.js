...
function spawnPhantom(site, path, options, cb) {
  ...
  var phantomProc = childProcess.spawn(options.phantomPath, phantomArgs, ['pipe', 'pipe', process.stderr]);

  phantomProc.stdout.on('data', function(msg){
    msg = msg.toString();
    if (msg.length && msg.substring(0, 10) === '[results] ') {
      console.log('[results]: ', JSON.parse(msg.slice(10)));
    } else {
      console.log('[phantomjs] ', msg.toString());
    }
    // todo: use emit to send message back to app.js?
  });
